...
export default function JsonFormatter() {
  ...
  const formatJson = useCallback((jsonString: string, formatMode: FormatMode): string => {
    try {
      ...
      throw new Error(`${t("invalidJson")}: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
  }, [indentSize, t]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* File Upload */}
      <div className="space-y-2">
        <label className="block text-sm font-medium">{t("uploadJsonFile")}</label>
        <input
          ref={fileInputRef}
          type="file"
          accept=".json"
      ...
      <div className="flex flex-wrap gap-4 items-center">
        ...
        {mode === "prettify" && (
          <div className="flex items-center gap-2">
            <label className="text-sm">{t("indent")}</label>
            <select
              value={indentSize}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold">{t("inputJson")}</h2>
          ...
          <Button
            ...
            className="w-full"
          >
            {mode === "prettify" && t("prettifyJson")}
            {mode === "minify" && t("minifyJson")}
            {mode === "validate" && t("validateJson")}
          </Button>
        </div>
      ...
      {/* Additional Tools */}
      <div className="space-y-3">
        <h2 className="text-lg font-semibold">{t("additionalTools")}</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          >
            <div className="font-medium">{t("compress")}</div>
            <div className="text-xs text-neutral-500">{t("compressDesc")}</div>
          </button>
          ...
          <button
            ...
            className="p-3 rounded-lg border border-neutral-300 dark:border-neutral-700 hover:bg-neutral-50 dark:hover:bg-neutral-900 disabled:opacity-50 text-sm"
          >
            <div className="font-medium">{t("escape")}</div>
            <div className="text-xs text-neutral-500">{t("escapeDesc")}</div>
          </button>
          ...
          <button
            ...
            className="p-3 rounded-lg border border-neutral-300 dark:border-neutral-700 hover:bg-neutral-50 dark:hover:bg-neutral-900 disabled:opacity-50 text-sm"
          >
            <div className="font-medium">{t("unescape")}</div>
            <div className="text-xs text-neutral-500">{t("unescapeDesc")}</div>
          </button>
          ...
          <button
            ...
            className="p-3 rounded-lg border border-neutral-300 dark:border-neutral-700 hover:bg-neutral-50 dark:hover:bg-neutral-900 disabled:opacity-50 text-sm"
          >
            <div className="font-medium">{t("extractKeys")}</div>
            <div className="text-xs text-neutral-500">{t("extractKeysDesc")}</div>
          </button>
        </div>
      </div>
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
        <h3 className="text-sm font-semibold text-blue-900 dark:text-blue-100 mb-2">{t("tips")}</h3>
