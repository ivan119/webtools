...
async function fetchUrlAsFile(url: string, t: any): Promise<File> {
  ...
  if (!contentType.includes("image/avif")) {
    throw new Error(t("errors.urlMustBeAvif"));
  }
...
export default function AvifToJpgConverter() {
  const t = useTranslations("avifToJpg");
      ...
      setQueue((prev) => {
        ...
        const toAdd = arr.slice(0, remainingSlots).map((file, idx) => {
          ...
          if (!file.type.includes("image/avif")) {
            return { id, file, error: t("errors.onlyAvif") } as QueuedFile;
          }
  ...
  const handleConvert = useCallback(async () => {
    ...
    for (let i = 0; i < updatedQueue.length; i++) {
      ...
      try {
        const jpgBlob = await convertAvifToJpg(item.file, 0.92, t);
        ...
        updatedQueue[i] = {
          ...item,
          error: t("errors.conversionFailedWithNote"),
        };
      }
    }
