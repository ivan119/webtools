...
function convertWebpToPng(file: File): Promise<Blob> {
...
export default function WebpToPngConverter() {
  ...
  const handleConvert = useCallback(async () => {
    ...
    for (let i = 0; i < updatedQueue.length; i++) {
      ...
      try {
        const pngBlob = await convertWebpToPng(item.file);
        const fname = item.file.name.replace(/\.webp$/i, ".png");
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        <h1 className="text-2xl font-semibold">WebP to PNG Converter</h1>
        <p className="text-sm text-neutral-600 dark:text-neutral-300">Convert WebP images to PNG format quickly in your browser.</p>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm text-neutral-500">Your PNG files will appear here, once you convert them.</span>
      </div>
            ...
            <li key={q.id} className="rounded-md border border-neutral-200 dark:border-neutral-800 p-3 space-y-2">
              ...
              {q.error ? (
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <a href={q.resultUrl} download={q.resultName || "converted.png"} className="text-sm underline">Download PNG</a>
                </div>
              ) : (
