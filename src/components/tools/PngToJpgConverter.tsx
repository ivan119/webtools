...
const MAX_FILES = 20;
const MAX_SIZE_BYTES = 20 * 1024 * 1024; // 20MB for PNG
...
function convertPngToJpg(file: File, quality: number = 0.92): Promise<Blob> {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      // Draw PNG onto white background to avoid black for transparent pixels
      if (ctx) {
    ...
    setQueue((prev) => {
      ...
      const toAdd = arr.slice(0, remainingSlots).map((file, idx) => {
        ...
        if (!file.type.includes("image/png")) {
          return { id, file, error: "Only PNG files are supported" } as QueuedFile;
        }
  ...
  const handleConvert = useCallback(async () => {
    ...
    for (let i = 0; i < updatedQueue.length; i++) {
      ...
      try {
        ...
        const fname = item.file.name.replace(/\.png$/i, ".jpg");
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        <h1 className="text-2xl font-semibold">PNG to JPG Converter</h1>
        <p className="text-sm text-neutral-600 dark:text-neutral-300">
          Convert PNG images to JPG for smaller file sizes. Transparent areas
      ...
      >
        <div className="text-sm">Drop your .png files here, or click to select them manually!</div>
        <div className="text-xs text-neutral-500">Up to 20 PNG files, max 20MB each.</div>
      </FileUpload>
